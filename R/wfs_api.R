...
#' @importFrom httr user_agent modify_url status_code http_status http_error
#'
#' @export
wfs_api <- function(base.url = NULL, queries) {
  ...
  # Set the user agent
  ua <- user_agent("https://github.com/pitkant/turku")

  # Construct the query URL
  url <- modify_url(base.url, query = queries)
  ...
  if (http_error(resp)) {
    ...
    exception_texts <- ""
    if (status_code == 400) {
      exception_texts <- xml_text(xml_find_all(content, "//ExceptionText"))
